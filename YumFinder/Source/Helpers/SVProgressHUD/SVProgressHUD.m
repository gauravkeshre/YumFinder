...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if ((self = [super initWithFrame:frame]))
    {
        ...
        SVProgressHUDBackgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"whiteColor.png"]];
        SVProgressHUDForegroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"green.png"]];
        if ([UIFont resolveClassMethod:@selector(preferredFontForTextStyle:)])
            ...
            SVProgressHUDFont = [UIFont systemFontOfSize:12];
        UIImage *imgSuccess = [UIImage imageNamed:@"SVProgressHUD.bundle/success"];
        UIImage *imgFailure = [UIImage imageNamed:@"SVProgressHUD.bundle/error"];
        if ([imgSuccess respondsToSelector:@selector(imageWithRenderingMode:)])
        {
            SVProgressHUDSuccessImage = [imgSuccess imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
            SVProgressHUDErrorImage = [imgFailure imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        }
        else
        {
            SVProgressHUDSuccessImage = imgSuccess;
            SVProgressHUDErrorImage = imgFailure;
        }
...
- (void)updatePosition
{
    ...
    if(string)
    {
        ...
        {
            stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:constraintSize lineBreakMode:NSLineBreakByCharWrapping];
            stringWidth = stringSize.width;
            stringHeight = ceil(stringSize.height);
        }
        if (imageUsed)
...
- (UILabel *)stringLabel
{
    if (_stringLabel == nil)
    {
      ...
      //  _stringLabel.lineBreakMode = NSLineBreakByWordWrapping;
